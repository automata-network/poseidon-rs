...
mod tests {
    ...
    fn test_hash() {
        ...
        let big_arr: &[Fr] = &[b1, b2, b0, b0, b0, b0];
        let h = poseidon.hash_fixed(big_arr).unwrap();
        assert_eq!(
            h.to_string(),
        ...
        let big_arr: &[Fr] = &[b3, b4, b0, b0, b0, b0];
        let h = poseidon.hash_fixed(big_arr).unwrap();
        assert_eq!(
            h.to_string(),
        ...
        let big_arr: &[Fr] = &[b1, b2, b3, b4, b5, b6];
        let h = poseidon.hash_fixed(big_arr).unwrap();
        assert_eq!(
            h.to_string(),
        ...
        let big_arr: &[Fr] = &[b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b13, b14];
        let h = poseidon.hash_fixed(big_arr).unwrap();
        assert_eq!(
            h.to_string(),
        ...
        let big_arr: &[Fr] = &[b1, b2, b3, b4, b5, b6, b7, b8, b9, b0, b0, b0, b0, b0];
        let h = poseidon.hash_fixed(big_arr).unwrap();
        assert_eq!(
            h.to_string(),
        ...
        let ret_ref = poseidon.hash_fixed(&[b0, b0]).unwrap();
        assert_eq!(
            format!("{}", ret_ref.as_u256()),
            "14744269619966411208579211824598458697587494354926760081771325075741142829156"
        );

        let h = poseidon.hash_with_cap(&[b0], 3, 0).unwrap();
        assert_eq!(h, ret_ref);
    }
